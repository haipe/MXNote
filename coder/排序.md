...
```
## 交换排序
### 主要思想：通过“交换”无效队列中的记录，从而得到其中关键字或者最大或者最小的记录，并将它加入到有序子队列中，以此方法增加记录的有序子队列的长度。
### 代表排序算法：冒泡排序，快速排序
```

//冒泡排序
//遍历，将最大或最小的数据，交换到对头或者尾部，逐步得到有序的子队列
template<typename array_type>
...
//优化 -----
//快速排序
//循环遍历一个基准数的位置，并且把大于基准的放在右边，把小于基准的放在左边
//然后递归的在基准位置的左边和右边区域，重复上一步
...
```
## 基数排序
### 如果需要排序当数据有多个排序基准当时候，采取对应当基数排序
### 比如：对学生的，院/系/学号排序的时候，各个基数为院/系/学号。先把队列按照【院】排序，再把队列按照【系】排序，再按照【学号】排序
```
//代码详见 test_cardinality_sort.cpp

```
## 外部排序
### 对于一次无法加载所有数据的情况，在外部分批排序，然后归并，从而得到排序数据
### 经典题目：将一个文件里面的一亿数字排序。
```
```