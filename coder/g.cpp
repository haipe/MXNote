...
int main()
{
    ...
    // vector_int out(ls1.size() + ls2.size(), 0);

    // int run_cnt = merge_sort_array_int(ls1.begin(), ls1.size(), ls2.begin(), ls2.size(), out, out.begin());
    // std::cout << "run count:" << run_cnt << std::endl;
    // printf_array_int(out);

    //vector_int vc = {1,3,2,5,6,9,8,0,14,12,11,15,13,4,7};//
